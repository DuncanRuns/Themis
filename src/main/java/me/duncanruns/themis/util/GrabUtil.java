...
public final class GrabUtil {
    ...
    private static HttpClient getHttpClient() throws NoSuchAlgorithmException, KeyStoreException, KeyManagementException {
        ...
        try {
            ...
            ThemisMod.LOGGER.warn("Outdated Java, GrabUtil is using an insecure HttpClient!");
        }
    ...
    public static Optional<String> grabPageTitle(String origin) throws IOException {
        String html = grab(origin);
        Matcher matcher = TITLE_PATTERN.matcher(html);
        if (matcher.find()) {
            String title = matcher.group(1);
            if (title == null) return Optional.empty();
            return Optional.ofNullable(StringEscapeUtils.unescapeHtml4(title.trim()));
        }
